/**
 * This class handles the displaying a list of assignments filtered by their difficulty level. 
 * It also handles potential errors during data fetching, displaying an error message.
 * It also shows with a table listing the assignments matching the selected difficulty level, 
...
const FilterByDifficulty = () => {
    ...
    const { difficulty_level } = useParams();
    const navigate = useNavigate();

    useEffect(() => {
        const fetchAssignmentsByDifficulty = async () => {
            try {
                const response = await axios.get(`http://localhost:3001/assignmentPlanner/difficulty/${difficulty_level}`);
    ...
    return (
        <div className="filter-container">
            <h2 className="filter-header">Assignments Filtered by Difficulty: {difficulty_level}</h2>
            ...
            {assignments.length > 0 ? (
                ...
                <div>No assignments found for the specified difficulty level.</div>
            )}
